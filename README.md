 :dart: **Objectif** : conception d'une api qui utilise du machine learning (prédiction du Titanic)
...
A chaque modification du package, il faut:   
- soit taper la commande pip install .  
- soit installer le module qui permet d'initialiser automatiquement le package :  
...
4. Faire tourner le serveur en local avec uvicorn : 

	>uvicorn nomdufichierdelapi:nomdelinstance --reload  
...
1. Le front (Javascript) récupère les réponses json de l'api:  
...
2. Pour tester en local la page, lancer le fichier index.html
...
3. Créer l'image : 
     ...
     :warning: ne pas oublier le point à la fin  
     :warning: si mise en prod avec Azure, appeler l'image du même nom que le container créé sur Azure (Login Server)  
	 
4. Exécute l'image en local : 

	>docker run -p 8080:8000 nom_de_l_image
...
![schema_ACR](https://cdn.discordapp.com/attachments/836540967272185866/877492047232241664/schema_ACR_3.png)
...
3. Envoyer l'image créée avec Docker vers l'ACR :  
	- Aller sur le Container registry (titanicpackagegna) and récupérer le Login server dans Access keys (Admin user must be "Enabled") + le password
	- Se connecter au registre via le Terminal : 
	
		>docker login name_of_LoginServer** (and indicate password)
	...
	- Pusher l'image dans l'ACR :
	
		>docker push nom_image/app_api
...
4. Déployer l'image disponible dans l'ACR via Web App  
	...
	- Dans Home, ouvrir l'onglet de l'App service, la run et cliquer sur l'url :globe_with_meridians:
...
2. Compilation/création du package (les métadonnées build / dist se génèrent)
